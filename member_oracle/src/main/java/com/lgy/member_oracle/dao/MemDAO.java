...
@Repository  // 이 클래스가 DAO(데이터 접근 객체) 역할임을 스프링에 알려주는 어노테이션
public class MemDAO
{
	DataSource dataSource; // DB 연결을 위한 DataSource 객체 선언 (커넥션풀 사용)

	// 생성자: 클래스가 생성될 때 데이터베이스 연결 준비
	public MemDAO()
	{
		try
		{
			// JNDI 방식으로 context 객체를 생성
